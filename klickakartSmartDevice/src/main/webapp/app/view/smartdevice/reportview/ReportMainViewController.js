Ext.define('Klickakart.view.smartdevice.reportview.ReportMainViewController',{
	extend :'Ext.app.ViewController',
	alias:'controller.report-mainview',
		...
		var button = Ext.widget('button', {
		    ...
		    listeners:{
		    	click :function( floatOp, width, height, eOpts ){
		    		var mainview = this.scope.view.up('report-mainview');
	...
	configureReportUI : function(scope,reportUIJson,reportmainview){
		...
		if(reportUIJson.hasDataPoint){
			var datapointview = Ext.create('Klickakart.view.smartdevice.reportview.datapointview.DataPointView',{
		...
		if(reportUIJson.hasGrid){
			...
			var dataviewpanel = Ext.create('Klickakart.view.smartdevice.reportview.dataview.DataView',{
				title : 'Data',
				iconAlign:'top',
				reportUIJson:reportUIJson
		...
		if(reportUIJson.hasChart){
			var chartviewpanel = Ext.create('Klickakart.view.smartdevice.reportview.chartview.ChartView',{
				title : 'Chart',
				iconAlign:'top',
				reportUIJson:reportUIJson
			//	icon:'resources/appicons/chart.png'
		...
		if(reportUIJson.searchCriteriaWidgets.length>0){
			var criteriaview = Ext.create('Klickakart.view.smartdevice.reportview.querycriteria.QueryCriteria',{
		...
		if(reportUIJson.hasMap){
			var mapviewpanel = Ext.create('Klickakart.view.smartdevice.reportview.mapview.MapView',{
				title : 'Map',
				reportUIJson:reportUIJson
			});

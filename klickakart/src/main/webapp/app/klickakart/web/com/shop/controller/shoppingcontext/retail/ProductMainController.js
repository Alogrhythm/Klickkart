Ext.define('Klickakart.klickakart.web.com.shop.controller.shoppingcontext.retail.ProductMainController', {
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#ProductTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#ProductForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#ProductGrid');
               var tree = this.view.down('#ProductTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#ProductForm');
          var grid = this.view.down('#ProductGrid');
          var tree = this.view.down('#ProductTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#ProductGrid');
               ...
               var formPanel = this.getView().up().down('#ProductForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#ProductTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#ProductForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#ProductGrid').store;
          gridStore.removeAll();

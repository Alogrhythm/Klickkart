Ext.define('Klickakart.klickakart.web.com.shop.controller.shoppingcontext.retail.ItemMainController', {
     ...
     productIdLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               success: function(response, scope) {
                    var compRef = scope.sender.down('#productId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'productId', response.responseText);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#ItemGrid');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#ItemForm').down('#' + me.column.dataIndex + '').store;

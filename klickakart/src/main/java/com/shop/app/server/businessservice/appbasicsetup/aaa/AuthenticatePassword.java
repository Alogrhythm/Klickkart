...
import com.shop.app.util.PasswordGenerator;
	...
	public String validatePassword(String password, PasswordPolicyInterface pwdPolicy) throws Exception {
			if (pwdPolicy != null) {
				PasswordGenerator passwordGenerator = new PasswordGenerator(
						pwdPolicy.getMinPwdLength(), pwdPolicy.getMaxPwdLength());
				if (pwdPolicy.getAllowedSpecialLetters() != null
						&& pwdPolicy.getAllowedSpecialLetters().length() > 0) {
					char[] charArray = pwdPolicy.getAllowedSpecialLetters()
							.toCharArray();
					Character[] speChars = ArrayUtils.toObject(charArray);
					passwordGenerator.setSPECIAL_LETTERS(speChars);
				}
				PasswordLogic[] pwdLogic = new PasswordLogic[] {
						new PasswordLogic('C', pwdPolicy.getMinCapitalLetters()),
						new PasswordLogic('S', pwdPolicy.getMinSmallLetters()),
						new PasswordLogic('N', pwdPolicy.getMinNumericValues()),
						new PasswordLogic('P', pwdPolicy.getMinSpecialLetters()) };
				if (!passwordGenerator.validatePwdData(pwdLogic)) {
					return "Incorrect Password Policy Configuration Data Found.";
				}
				String msg = passwordGenerator.validatePassword(pwdLogic,
						password);

package com.shop.app.server.repository.datavisualizer.searchengine.aspect;
...
@Aspect
@Component
public class RepositorySearchEngineAspect extends RepositoryAspect {
    ...
    public Object aroundAllOtherOpeartion(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        try {
            Log.out.println("DVISE354901100", runtimeLogInfoHelper.getRequestHeaderBean(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName(),joinPoint.getSignature().getDeclaringTypeName(),joinPoint.getSignature().getName());
            object = joinPoint.proceed();
            methodCallDetails.setPostCallDetails(HealthConstants.METHOD_NORMAL_EXECUTION);
            Log.out.println("DVISE354901100", runtimeLogInfoHelper.getRequestHeaderBean(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName(),joinPoint.getSignature().getDeclaringTypeName(),joinPoint.getSignature().getName());
        } catch (Exception e) {
            Log.out.println("DVISE354900100", runtimeLogInfoHelper.getRequestHeaderBean(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName(), e.getMessage());
            throw e;
         } finally {
     ...
     @Pointcut("execution(* com.athena.search..repository..*(..)) && !within(com.athena.server.dataengine.repository.ArtQueryRepository+)")
